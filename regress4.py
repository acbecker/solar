...
if __name__ == "__main__":
    ...
    for mKey in mesonets.keys():
        ...
        for eKey in range(11): # which element
            for hKey in range(5): # which hour
                ...
                for f in range(len(fKeys)):
                    fKey         = fKeys[f]
                    feattr[:,f]  = train[mKey].pdata[pKey::stride][fKey][:-nCv]
                    featcv[:,f]  = train[mKey].pdata[pKey::stride][fKey][-nCv:]
                if useAstro:
        ...
        for hKey in range(5): # which hour
            ...
            for f in range(len(fKeys)):
                fKey        = fKeys[f]
                data        = np.ravel(np.mean(train[mKey].pdata[fKey].reshape((NPTSt, 11, 5)), axis=1))[hKey::hstride]
                feattr[:,f] = data[:-nCv]
                featcv[:,f] = data[-nCv:]
            if useAstro:
                feattr[:,len(fKeys)]    = mesonets[mKey].datat["sun_alt"][:-nCv]
    ...
    for eKey in range(11): # which element
        for hKey in range(5): # which hour
            ...
            for mKey in mesonets.keys():
                for f in range(len(fKeys)):
                    fKey       = fKeys[f]
                    feattr[fIdx*nTr:(fIdx*nTr + nTr),f] = train[mKey].pdata[pKey::stride][fKey][:-nCv]
                    featcv[fIdx*nCv:(fIdx*nCv + nCv),f] = train[mKey].pdata[pKey::stride][fKey][-nCv:]
                if useAstro:
    ...
    for hKey in range(5): # which hour
        ...
        for mKey in mesonets.keys():
            for f in range(len(fKeys)):
                fKey       = fKeys[f]
                avg        = np.ravel(np.mean(train[mKey].pdata[fKey].reshape((NPTSt, 11, 5)), axis=1))[hKey::hstride]
                feattr[fIdx*nTr:(fIdx*nTr + nTr),f] = avg[:-nCv]
                featcv[fIdx*nCv:(fIdx*nCv + nCv),f] = avg[-nCv:]
