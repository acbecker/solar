...
if __name__ == "__main__":
    ...
    suffix     = sys.argv[1]
    useBoxcox  = int(sys.argv[2])
    useAstro   = 1
    if useBoxcox:
        trainFile  = "gp5_bc_train_%s.pickle" % (suffix)
        predFile   = "gp5_bc_test_%s.pickle" % (suffix)
    else:
        trainFile  = "gp5_train_%s.pickle" % (suffix)
        predFile   = "gp5_test_%s.pickle" % (suffix)
