...
This does box-cox.  This one actually goes back to 2-d, so this is gp4 with 2-d
"""
...
if __name__ == "__main__":
    ...
    for key in fKeys:
        ...
        for latidx in range(9):
            for lonidx in range(16):
                #data  = boxcox(f[fMapper[key]][:,:,:,latidx,lonidx])
                data  = f[fMapper[key]][:,:,:,latidx,lonidx]
                dataC = sigclip(data[:,:,:], True)
                gefsC = gefssC[sidx]
...
#        gelevs.append(gefs.elev)
    ...
    for regr in ("constant", "linear", "quadratic"):
        ...
        datafile = "gp5_%s_%s.pickle" % (switch, regr)
